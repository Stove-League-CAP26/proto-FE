...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-50" style={{ fontFamily: "'Noto Sans KR', sans-serif" }}>
      ...
      <nav className="bg-white border-b border-gray-100 sticky top-0 z-50 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 flex items-center h-14 gap-1">
          ...
          >
            ...
            <span className="font-black text-gray-900 text-lg tracking-tight">스토브리그</span>
          </button>
          {NAV_ITEMS.map((item) => (
            <button
              key={item.id}
      ...
      {currentPage === "team"    && <TeamPage onSelectPlayer={() => setCurrentPage("player")} />}

      {/* 푸터 */}
      <footer className="border-t border-gray-100 bg-white mt-8">
        <div className="max-w-6xl mx-auto px-4 py-4 flex justify-between">
          <p className="text-xs text-gray-400">⚾ 스토브리그 · KBO 야구 팬 플랫폼 프로토타입</p>
